...
- 项目编号：`hwj-gowork-104`
- 项目名称：晶圆制程批次与配方版本隔离服务
- 项目说明：基于 Go HTTP JSON 与 SQLite 文件持久化，管理晶圆批次、工艺路线版本、设备资质、配方快照、制程运行、量测结果和暂扣处置。
...
```bash
./build_benzhi_docker.sh hwj-gowork-104:benzhi-amd64 linux/amd64
./build_benzhi_docker.sh hwj-gowork-104:benzhi-arm64 linux/arm64
```

## 运行

```bash
docker run --rm -it --network none hwj-gowork-104:benzhi-amd64 bash
```
